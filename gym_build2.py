...
log_data = []  # List to store log data
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("gym.log", mode="a", encoding="utf-8"),
...
def save_log_data():
    try:
        ...
        logging.error("Error during pickling object (Possibly unsupported): %s", ex)
...
def load_log_data():
    ...
    try:
        ...
        logging.error("Error during unpickling object (Possibly unsupported): %s", ex)
...
def signup() -> bool:  # account signup verification
    ...
    if username in user_accounts:
        ...
        logging.info(f"Signup attempt failed for username: {username} (already exists)")
        return False
    password = input(
    ...
    if len(password) < 6:
        ...
        logging.info(
            f"Signup attempt failed for username: {username} (password too short)"
        )
        return False
    if not re.search(r"[A-Z]", password):
        print("[+]Error 1002: pass must contain one uppercase")
        logging.info(
            f"Signup attempt failed for username: {username} (password missing uppercase)"
        )
        return False
    if not re.search(r"[a-z]", password):
        print("[+]Error 1003: pass must contain at least one lowercase")
        logging.info(
            f"Signup attempt failed for username: {username} (password missing lowercase)"
        )
        return False
    if not re.search(r"\d", password):
        print("[+]Error 1004: pass must contain one digit")
        logging.info(
            f"Signup attempt failed for username: {username} (password missing digit)"
        )
        return False
    if not re.search(r"[!@#$%^&*(),.?\":{}|<>]", password):
        print("[+]Error 1005: pass must contain one special char")
        logging.info(
            f"Signup attempt failed for username: {username} (password missing special character)"
        )
        return False
    ...
    log_data.append(
        {"action": "signup", "username": username, "timestamp": datetime.now()}
    )
    save_log_data()
    return True
    ...
    while True:
        ...
        if user_data["lockout_time"]:
            if datetime.now() < user_data["lockout_time"]:
                ...
                logging.info(
                    f"Login attempt failed for username: {username} (account locked)"
                )
                continue
            else:
        ...
        if username in user_accounts and user_accounts[username] == password:
            ...
            log_data.append(
                {"action": "login", "username": username, "timestamp": datetime.now()}
            )
            save_log_data()
            return True
        else:
            ...
            logging.info(
                f"Login attempt failed for username: {username} (invalid credentials)"
            )

            if user_data["attempts"] >= max_attempts:
                print("[+]Too many failed attempts. The system will logout.")
                user_data["lockout_time"] = datetime.now() + lockout_duration
                logging.info(
                    f"Account locked for username: {username} due to too many failed attempts"
                )
                return False
...
def member_checkin() -> None:
    ...
    if mem_ID not in mem_login:
        ...
        logging.info(f"Member check-in attempt failed for ID: {mem_ID} (invalid ID)")
        return
    checkin_time = datetime.now()
    check_in.append({"member_id": mem_ID, "timestamp": checkin_time, "sessions": []})
    logging.info(f"Member check-in successful for ID: {mem_ID} at {checkin_time}")
    log_data.append(
        {"action": "member_checkin", "member_id": mem_ID, "timestamp": checkin_time}
    )
    save_log_data()
    ...
    while True:
        ...
        if usr_choice in sess_inf:
            ...
            logging.info(f"Member {mem_ID} registered for session {usr_choice}")
            log_data.append(
                {
                    "action": "session_register",
                    "member_id": mem_ID,
                    "session_id": usr_choice,
...
def add_member():
    ...
    if confirm != "Y":
        ...
        logging.info(f"Add member attempt cancelled for {mem_ID}")
        return
    ...
    logging.info(f"New member added: {mem_ID} ({username} {username_last})")
    log_data.append(
        {
            "action": "add_member",
            "member_id": mem_ID,
...
def sess_mang():
    ...
    if user_prompt == "1":
        ...
        logging.info(f"New session added: {sess_ID} ({name_s}, ${co}, {sched})")
        log_data.append(
            {
                "action": "add_session",
                "session_id": sess_ID,
        ...
        if sess_ID in sess_inf:
            ...
            new_sched = input("[+]Your new schedule: ").strip().title()
            sess_inf[sess_ID]["cost"] = new_cost
            sess_inf[sess_ID]["sched"] = new_sched
            logging.info(
                f"Session updated: {sess_ID} (new cost: ${new_cost}, new schedule: {new_sched})"
            )
            log_data.append(
                {
...
def add_instructor():
    ...
    logging.info(f"New instructor added: {instr_ID} ({selected})")
    log_data.append(
        {
